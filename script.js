const SHEET_ID = "1X5rx_VAk6gvjNX2UHFZKFe-v_SexMadiAtc3THe_jns";
const SHEET_GID = "0";
// FREE static-hosting version: reads the published Google Sheet directly.
...
async function loadProducts() {
  try {
    ...
    products = rows.slice(1).map((row, rowIndex) => {
    ...
    $("sheetStatus").textContent = "We could not load the current inventory. Publish your Google Sheet to the web, then refresh.";
...
function bottleClass(category) {
  return `bottle bottle--${category.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
}
...
function renderProducts() {
  ...
  if (sheetError) {
    ...
    $("emptyState").querySelector("p").textContent = "Publish the Google Sheet to the web, then refresh.";
  ...
  $("productGrid").innerHTML = list.map((product) => `
    <article class="product-card">
      <div class="product-image"><div class="${bottleClass(product.category)}" aria-hidden="true"><span>${escapeHtml(product.category)}</span></div></div>
      <div class="product-body">
        ...
        <div class="product-bottom"><span class="price">${money(product.price)}</span>
          <span class="stock-status stock-status--${product.inventory === 0 ? "out" : product.inventory < 10 ? "low" : "in"}">${product.inventory === 0 ? "Out of stock" : product.inventory < 10 ? `${product.inventory} left` : "In stock"}</span>
          <button class="add-button" type="button" data-add="${product.id}" ${product.inventory === 0 ? "disabled" : ""}>${product.inventory === 0 ? "Out of stock" : "Add to cart"}</button>
        </div>
      </div>
    </article>
  `).join("");
...
function renderCart() {
  if (!cart.length) {
    ...
    $("cartItems").innerHTML = cart.map((item) => {
      ...
      return `
        <div class="cart-item">
          <div><strong>${escapeHtml(productDisplayName(product))}</strong><div class="cart-item-meta">${escapeHtml(product.type || product.brand)} · ${money(product.price)} each</div>
          ...
          </div><strong>${money(product.price * item.qty)}</strong>
        </div>`;
    }).join("");
  }
